...
import {homeV3} from '../src/home-v3.mjs';
...
export function build(){
 ...
 for(const name of ['styles.css','refinements.css','client.js','client-base.js','home-v3.css','home-v3.js'])fs.copyFileSync(path.join(root,'src',name),path.join(out,'assets',name));
 fs.copyFileSync(path.join(root,'public/fonts/fonts.css'),path.join(out,'assets/fonts.css'));
 const routes=[];
 function write(route,html){
  const relative=route==='/'?'':route.replace(/^\//,'');
  const file=path.join(out,relative,'index.html');
  fs.mkdirSync(path.dirname(file),{recursive:true});
  const prefix=path.relative(path.dirname(file),out).split(path.sep).join('/')||'.';
 ...
 write('/',homeV3());
 write('/writing/e30-respray/',T.series());
 ...
 write('/writing/',refineWriting(T.writing()));
 write('/photography/',T.photography());
 ...
 console.log(`Built ${routes.length} pages in dist/`);return routes;
}
