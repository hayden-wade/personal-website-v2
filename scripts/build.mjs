...
export function build(){fs.rmSync(out,{recursive:true,force:true});fs.mkdirSync(out,{recursive:true});fs.cpSync(path.join(root,'public'),out,{recursive:true});fs.mkdirSync(path.join(out,'assets'),{recursive:true});for(const name of ['styles.css','client.js','client-base.js'])fs.copyFileSync(path.join(root,'src',name),path.join(out,'assets',name));fs.copyFileSync(path.join(root,'public/fonts/fonts.css'),path.join(out,'assets/fonts.css'));
const routes=[];
